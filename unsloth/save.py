...
def save_to_gguf(
    ...
    final_location = str((Path(model_directory) / f"bgGPT-llama-3.2-3B-Instruct.{first_conversion.upper()}.gguf").absolute())
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            ...
            final_location = str((Path(model_directory) / f"bgGPT-llama-3.2-3B-Instruct.{quant_method.upper()}.gguf").absolute())
