...
def save_to_gguf(
    ...
    final_location = str((Path(model_directory) / f"bgGPT-llama-3.1-8B-math.{first_conversion.upper()}.gguf").absolute())
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            ...
            final_location = str((Path(model_directory) / f"bgGPT-llama-3.1-8B-math.{quant_method.upper()}.gguf").absolute())
